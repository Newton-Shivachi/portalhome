...
urlpatterns = [
    ...
    path('<int:house_id>/', house_detail, name='house_detail'),path("houses/<int:house_id>/", view_house, name="view_house"),
    path('add/', add_house, name='add_house'),path("login/", LoginView.as_view(template_name="houses/login.html"), name="login"),
    path('<int:house_id>/add_images/', add_house_images, name='add_house_images'),path("logout/", logout_view, name="logout"),
